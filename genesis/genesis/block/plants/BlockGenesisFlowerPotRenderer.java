...
public class BlockGenesisFlowerPotRenderer implements ISimpleBlockRenderingHandler {
			...
			Block block, int modelID, RenderBlocks renderer)
	{
		...
		renderer.renderFaceYPos(block, x, y - 0.5F + offset + 0.1875F, z, renderer.getBlockIcon(Block.dirt));
		...
		if (metadata != 0)
		{
			...
			if (plantBlockAndMeta != null)
			{
				...
				Block plantBlock = Block.blocksList[plantBlockAndMeta.blockID];
				...
				else
				{
					...
					Icon plantIcon = iPlant.getIconForFlowerPot(world, x, y, z, plantBlockAndMeta.metadata);
					...
					renderer.drawCrossedSquares(plantBlock, plantBlockAndMeta.metadata,
							x, y, z, iPlant.renderScale(world, x, y, z));
