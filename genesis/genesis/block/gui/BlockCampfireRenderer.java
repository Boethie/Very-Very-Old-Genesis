...
import genesis.genesis.block.BlockTikiTorch;
import genesis.genesis.util.rendering.FaceHelper;
import genesis.genesis.util.rendering.Quad;
import genesis.genesis.util.rendering.FaceUVs;
import genesis.genesis.util.rendering.IShadingMethod;
import genesis.genesis.util.rendering.Matrix;
import genesis.genesis.util.rendering.RotateableBox;
import genesis.genesis.util.rendering.RotateableModelQuads;
import genesis.genesis.util.rendering.RotateableModelTris;
import genesis.genesis.util.rendering.Tri;
import genesis.genesis.util.rendering.Vec4;
...
public class BlockCampfireRenderer implements ISimpleBlockRenderingHandler {
			...
			Block block, int modelId, RenderBlocks renderer)
	{
		...
		Icon icon = renderer.getBlockIconFromSideAndMetadata(campfire, 0, metadata);
		...
		RotateableBox rock = new RotateableBox(0.25F, 0.25F, 0.25F,
				icon.getMinU(), icon.getMaxU(),
				icon.getMinV(), icon.getMaxV(), 16);
		rock.shadingMethod = shading;
		rock.translate(x + 0.5, y + (rock.height / 2), z + 0.5);
		
		rock.rotate(rockPosRot, up);
		
		int blockIDUnder = world.getBlockId(x, y - 1, z);
		Block blockUnder = Block.blocksList[blockIDUnder];
		int metadataUnder = world.getBlockMetadata(x, y - 1, z);
		boolean dirtyBlock = false;
		
		if (blockIDUnder != Block.dirt.blockID)
		{
			try
			{
				dirtyBlock = blockUnder.idDropped(metadataUnder, null, 0) == Block.dirt.blockID;
			}
		...
		FaceHelper.getNextInCircle(new Vec4(0, offY, 0, 1),
				FaceHelper.uvFromIcon(0.5, 0.5, icon));
		...
		for (int i = 0; i < count; i++)
		{
			...
			Tri tri = FaceHelper.getNextInCircle(new Vec4(offX, offY, offZ, 1),
					FaceHelper.uvFromIcon(0.5 + offX, 0.5 + offZ, icon));
		...
		tris[tris.length - 1] = FaceHelper.getNextInCircle(FaceHelper.verts.get(1),
				FaceHelper.uvs.get(1));
		...
		Quad xPos = new Quad(new Vec4(-distH, 0, -distH, 1),
							new Vec4(-distH, 0, distH, 1),
							new Vec4(-distH, height, distH, 1),
							new Vec4(-distH, height, -distH, 1),
							new FaceUVs(icon, 1, 3,
										0, 16));
		Quad zPos = new Quad(new Vec4(-distH, 0, distH, 1),
							new Vec4(distH, 0, distH, 1),
							new Vec4(distH, height, distH, 1),
							new Vec4(-distH, height, distH, 1),
							new FaceUVs(icon, 3, 5,
									0, 16));
		Quad xNeg = new Quad(new Vec4(distH, 0, distH, 1),
							new Vec4(distH, 0, -distH, 1),
							new Vec4(distH, height, -distH, 1),
							new Vec4(distH, height, distH, 1),
							new FaceUVs(icon, 5, 7,
									0, 16));
		Quad zNeg = new Quad(new Vec4(distH, 0, -distH, 1),
							new Vec4(-distH, 0, -distH, 1),
							new Vec4(-distH, height, -distH, 1),
							new Vec4(distH, height, -distH, 1),
							new FaceUVs(icon, 7, 9,
									0, 16));
		Quad yPos = new Quad(new Vec4(distH, height, -distH, 1),
							new Vec4(-distH, height, -distH, 1),
							new Vec4(-distH, height, distH, 1),
							new Vec4(distH, height, distH, 1),
							new FaceUVs(icon, 11, 13,
									5, 7));
		Quad yNeg = new Quad(new Vec4(-distH, 0, -distH, 1),
							new Vec4(distH, 0, -distH, 1),
							new Vec4(distH, 0, distH, 1),
							new Vec4(-distH, 0, distH, 1),
							new FaceUVs(icon, 11, 13,
		...
		RotateableModelQuads vertStick = new RotateableModelQuads(new Quad[]{
				xPos,
				zPos,
				xNeg,
				zNeg,
		...
		RotateableBox vStick = new RotateableBox(0.08838834764831844055010554526311F, 0.125F, 0.126F,
				icon.getMinU(), icon.getMaxU(),
				icon.getMinV(), icon.getMaxV(), 6);
		...
		if (lit)
		{
			icon = renderer.getBlockIconFromSideAndMetadata(Block.fire, 0, 0);
			
			distH = 0.375F;
			height = 1;
			float distT = 0.25F;
			float distB = 0.375F;
			FaceUVs iconUVs = new FaceUVs(icon.getMinU(), icon.getMaxU(),
					icon.getMinV(), icon.getMaxV());
			
			Quad xNegOut = new Quad(new Vec4(-distB, 0, -distH, 1),
									new Vec4(-distB, 0, distH, 1),
									new Vec4(-distT, height, distH, 1),
									new Vec4(-distT, height, -distH, 1),
									iconUVs);
			Quad xPosOut = new Quad(new Vec4(distB, 0, distH, 1),
									new Vec4(distB, 0, -distH, 1),
									new Vec4(distT, height, -distH, 1),
									new Vec4(distT, height, distH, 1),
									iconUVs);
			Quad zNegOut = new Quad(new Vec4(distH, 0, -distB, 1),
									new Vec4(-distH, 0, -distB, 1),
									new Vec4(-distH, height, -distT, 1),
									new Vec4(distH, height, -distT, 1),
									iconUVs);
			Quad zPosOut = new Quad(new Vec4(-distH, 0, distB, 1),
									new Vec4(distH, 0, distB, 1),
									new Vec4(distH, height, distT, 1),
									new Vec4(-distH, height, distT, 1),
									iconUVs);
			...
			Quad xNegIn = new Quad(new Vec4(-distB, 0, -distH, 1),
									new Vec4(-distB, 0, distH, 1),
									new Vec4(-distT, height, distH, 1),
									new Vec4(-distT, height, -distH, 1),
									iconUVs);
			Quad xPosIn = new Quad(new Vec4(distB, 0, distH, 1),
									new Vec4(distB, 0, -distH, 1),
									new Vec4(distT, height, -distH, 1),
									new Vec4(distT, height, distH, 1),
									iconUVs);
			Quad zNegIn = new Quad(new Vec4(distH, 0, -distB, 1),
									new Vec4(-distH, 0, -distB, 1),
									new Vec4(-distH, height, -distT, 1),
									new Vec4(distH, height, -distT, 1),
									iconUVs);
			Quad zPosIn = new Quad(new Vec4(-distH, 0, distB, 1),
									new Vec4(distH, 0, distB, 1),
									new Vec4(distH, height, distT, 1),
									new Vec4(-distH, height, distT, 1),
									iconUVs);
			
			RotateableModelQuads fire = new RotateableModelQuads(new Quad[]{
					xNegOut,
					xPosOut,
					zNegOut,
					zPosOut,
			...
			icon = Block.coalBlock.getIcon(0, 0);
			float coalWidth = 0.25F;
			RotateableBox coals = new RotateableBox(coalWidth, 0.125F, coalWidth,
					icon.getMinU(), icon.getMaxU(),
					icon.getMinV(), icon.getMaxV(), 16);
