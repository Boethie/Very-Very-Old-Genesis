package genesis.genesis.lib;

public class Names {
	
	public static final String blockLogAraucarioxylon_name = "Araucarioxlon Log";
	public static final String blockLogBjuvia_name = "Bjuvia Log";
	public static final String blockLogCordaites_name = "Cordaites Log";
	public static final String blockLogFig_name = "Fig Log";
	public static final String blockLogLepidodendron_name = "Lepidodendron Log";
	public static final String blockLogSigillaria_name = "Sigillaria Log";
	public static final String blockLogTempskya_name = "Tempskya Log";
	public static final String blockLogGenesis_name = "Genesis Log";
	public static final String blockLogGenesis_unlocalizedName = "blockLogGenesis";
	
	public static final String blockSaplingAraucarioxylon_name = "Araucarioxlon Sapling";
	public static final String blockSaplingBjuvia_name = "Bjuvia Sapling";
	public static final String blockSaplingCordaites_name = "Cordaites Sapling";
	public static final String blockSaplingFig_name = "Fig Sapling";
	public static final String blockSaplingLepidodendron_name = "Lepidodendron Log";
	public static final String blockSaplingSigillaria_name = "Sigillaria Sapling";
	public static final String blockSaplingTempskya_name = "Tempskya Sapling";
	public static final String blockSaplingGenesis_name = "Genesis Sapling";
	public static final String blockSaplingGenesis_unlocalizedName = "blockSaplingGenesis";
	
	public static final String blockLeavesAraucarioxylon_name = "Araucarioxlon Leaves";
	public static final String blockLeavesBjuvia_name = "Bjuvia Leaves";
	public static final String blockLeavesCordaites_name = "Cordaites Leaves";
	public static final String blockLeavesFig_name = "Fig Leaves";
	public static final String blockLeavesLepdodendron_name = "Lepdodendron Leaves";
	public static final String blockLeavesSigillaria_name = "Sigillaria Leaves";
	public static final String blockLeavesTempskya_name = "Tempskya Leaves";
	public static final String blockLeavesGenesis_name = "Genesis Leaves";
	public static final String blockLeavesGenesis_unlocalizedName = "blockLeavesGenesis";

}
