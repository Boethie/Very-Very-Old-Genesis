		...
		if (par4 >= 1 && par4 + l + 1 <= 256)
		{
			...
			else
			{
				...
				boolean isSoil = (soil != null && soil.canSustainPlant(par1World, par3, par4, par5, ForgeDirection.UP, 
						(BlockGenesisSapling)TreeBlocks.blocksSaplings[0]));
				if(isSoil && par4 < 256 - l - 1)
				{
					...
					for (j1 = 0; j1 < l; ++j1)
					{
						...
						{
							this.setBlockAndMetadata(par1World, par3, par4 + j1, par5, TreeBlocks.blocksLogs[0].blockID, this.metaWood);
						}
					}
					...
					for(int i4 = 2; i4 > -3; i4 -= 4)
					{
						for (j1 = par4 + l + 1; j1 <= par4 + l + 5; ++j1)
						{
							for(int k4 = 1; k4 > -3; k4 -= 2)
							{
								...
								{
									this.setBlockAndMetadata(par1World, par3 - i4, j1, par5 - k4, TreeBlocks.blocksLeaves[0].blockID, this.metaLeaves);
								}
							}
							for(int k4 = 1; k4 > -3; k4 -= 2)
							{
								...
								if (block == null || block.canBeReplacedByLeaves(par1World, j1, j1, par5 + 1))
								{
									this.setBlockAndMetadata(par1World, par3 - i4 - k4, j1, par5, TreeBlocks.blocksLeaves[0].blockID, this.metaLeaves);
								}
							}
							...
							Block block = Block.blocksList[k1];
							int id = TreeBlocks.blocksLogs[0].blockID;
							if(j1 + 1 > par4 + l + 5)
							{
								id = TreeBlocks.blocksLeaves[0].blockID;
							}
						...
						{
							this.setBlockAndMetadata(par1World, par3 + (i4 / 2), j1 - 6, par5, TreeBlocks.blocksLogs[0].blockID, this.metaWood);
						}
						...
						{
							this.setBlockAndMetadata(par1World, par3 - i4, j1, par5, TreeBlocks.blocksLeaves[0].blockID, this.metaLeaves);
						}
					}
