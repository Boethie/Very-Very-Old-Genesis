...
public class LanguageLoader {
	...
	public static final String LOCALIZATION_PATH = "assets/" + Genesis.MOD_ID + "/lang/";
	...
	public static void loadLanguages()
	{
		...
		try
		{
			langList = getResourceListing(Genesis.class, LOCALIZATION_PATH);
		}
		catch (Exception ex) {
			LogHelper.log("Failed to get list of language files in \"" + LOCALIZATION_PATH + "\".");
			ex.printStackTrace();
		}
		...
		for (String langFilename : langList)
		{
			try
			{
				...
				{
					LogHelper.log("Found but failed to load filename: " + langFilename);
				}
			}
			catch (Exception ex) {
				LogHelper.log("Language loading failed while attempting to load \"/" + LOCALIZATION_PATH + langFilename + "\"");
				ex.printStackTrace();
			}
		}
	}
