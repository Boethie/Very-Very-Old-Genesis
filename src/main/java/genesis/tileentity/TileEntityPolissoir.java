...
public class TileEntityPolissoir extends SyncedTileEntity implements ISidedInventory {
    ...
    public void readFromNBT(NBTTagCompound nbtTagCompound) {
        ...
        remainingUses = nbtTagCompound.getInteger("remainingUses");
    ...
    public void writeToNBT(NBTTagCompound nbtTagCompound) {
        ...
        nbtTagCompound.setInteger("remainingUses", remainingUses);
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote) {
            ...
            //TODO: Handle this better, needs particles and sound
            if(remainingUses <= 0)
            {
                for(int i = 0; i < this.polissoirItemStacks.length; i++)
                {
                    if(this.polissoirItemStacks[i] != null)
                    {
                        EntityItem item = new EntityItem(worldObj);
                        item.setLocationAndAngles(xCoord + 0.5D, yCoord + 0.8D, xCoord + 0.5D, 0, 0);
                        item.setEntityItemStack(this.polissoirItemStacks[i].copy());
