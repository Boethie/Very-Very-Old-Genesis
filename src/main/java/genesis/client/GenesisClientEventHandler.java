...
import genesis.block.plants.BlockCalamitesPlant;
import genesis.block.plants.BlockCalamitesPlant.CalamitesProperties;
	...
	public void addDebugText(RenderGameOverlayEvent.Text event) {
		...
		if (mc.gameSettings.showDebugInfo) {
			...
			if (lookPos != null && lookPos.typeOfHit == EnumMovingObjectType.TILE) {
				...
				Block block = mc.theWorld.getBlock(hitX, hitY, hitZ);
				...
				if (blockItem instanceof ItemBlock) {
					...
					locName = itemBlock.getItemStackDisplayName(pickStack);
				}
				...
				if (block instanceof BlockCalamitesPlant) {
					...
					extraStuff = String.format("height: %d, position: %d, eggs: %b, top: %b", props.height, props.position, props.hasEggs, props.top);
				}

				TileEntity tileEntity = mc.theWorld.getTileEntity(hitX, hitY, hitZ);
				...
				boolean solidOnSide = block.isSideSolid(mc.theWorld, hitX, hitY, hitZ, hitDir);
				...
				left.add("Block info:");
				left.add(String.format("bid: %s:%d, bname: %s, mat: %s", block.getUnlocalizedName(), metadata, locName == null ? "N/A" : "\"" + locName + "\"", material.getClass().getSimpleName()));
