...
public class TileEntityCampfire extends TileEntityFurnace {
	...
	public boolean canSmeltItemType(ItemStack stack)
	{
		...
		if (smeltResult != null)
		{
			...
			if (smeltResult.itemID == Item.coal.itemID)
				return true;
		}
	...
	public void updateEntity()
	{
		...
		if (!worldObj.isRemote)
		{
			...
			else if (furnaceBurnTime == 0)
			{
				...
				currentItemBurnTime = furnaceBurnTime = (int)getItemBurnTime(burningItem);
				...
				if (furnaceBurnTime > 0)
				{
					...
					if (burningItem != null)
					{
						...
						{
							setInventorySlotContents(1, burningItem.getItem().getContainerItemStack(burningItem));
						}
					}
				}
			}
			...
			if (wasBurning != burning)
			{
				worldObj.setBlockMetadataWithNotify(xCoord, yCoord, zCoord,
						blockCampfire.setFireLit(worldObj.getBlockMetadata(xCoord, yCoord, zCoord), burning), 2);
	...
	{
		return AxisAlignedBB.getAABBPool().getAABB(xCoord - 0.25, yCoord, zCoord - 0.25,
				xCoord + 1.25, yCoord + 1.25, zCoord + 1.25);
	}
